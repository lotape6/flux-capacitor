...
bash -n "${REPO_DIR}/install/completion/flux-completion.bash" || { echo "Bash completion script has syntax errors"; exit 1; }

# Test Zsh completion script (if zsh is available)
if command -v zsh >/dev/null 2>&1; then
    echo "Testing zsh completion script..."
    zsh -n "${REPO_DIR}/install/completion/flux-completion.zsh" || { echo "Zsh completion script has syntax errors"; exit 1; }
else
...
if [ -r "${REPO_DIR}/install/completion/flux-completion.fish" ]; then
