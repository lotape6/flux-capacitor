...
_flux_yaml_files_fzf() {
    ...
    local selected
    selected=$(print -l $files | fzf --height 40% --reverse --multi \
        --preview 'bat --color=always --style=numbers {} 2>/dev/null || cat {}' \
        --preview-window=right:60% --prompt="Select YAML files: ")
...
# compdef is only available after compinit; guard against being sourced early.
